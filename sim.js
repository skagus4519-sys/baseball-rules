...
function genThirdOut(){
  ...
  const kind = one([
    ...
    {t:"3루로 뛰던 2루 주자를 야수가 태그해 3아웃", force:false, why:"태그아웃",
      sets:[BASE_SETS[6], BASE_SETS[7]]}                        // 2·3루 / 만루 (2루 주자 필요)
  ]);
...
function genSacFly(){
  const outs = ri(0,2);
  const ok = outs < 2;
  return make({ tag:"희생플라이", outs, runners:one([BASE_SETS[3],BASE_SETS[5],BASE_SETS[6],BASE_SETS[7]]).r, rule:"sac",
    q:`${outsWord(outs)} 3루. 타자가 <b>외야 깊은 뜬공</b>을 쳤고 외야수가 잡았다. 3루 주자는 태그업해 홈으로 들어왔다면?`,
...
function genInterferenceType(){
  const c = one([
    {t:"타자주자가 러닝 레인을 벗어나 달리다 1루 송구를 맞혔다", off:true},
    {t:"주자가 병살을 막으려 베이스가 아닌 야수를 향해 파고들었다", off:true},
    {t:"타구를 처리하려는 야수의 진로를 주자가 가로막았다", off:true},
    {t:"공을 갖고 있지 않은 유격수가 2루로 가는 주자의 진로를 막았다", off:false},
    {t:"공을 기다리던 1루수가 베이스를 완전히 막고 서서 주자가 밟지 못했다", off:false}
  ]);
  return make({ tag:"방해 판정", outs:ri(0,2), runners:withRunner().r, rule: c.off?"interference":"obstruction",
    q:`${c.t}. 판정은?`,
...
function genBaseAward(){
  const c = one([
    ...
    {t:"1루 견제 악송구가 관중석으로 들어갔다", n:1, why:"볼데드 지역으로 들어간 송구는 보통 <b>2베이스</b>."},
  ...
  return make({ tag:"안전진루권", outs:ri(0,2), runners:withRunner().r, rule:"base-award",
    q:`${c.t}. 주어지는 진루는?`,
...
const CASES = [
  ...
  { tag:"한 베이스 두 주자", outs:1, runners:R(0,1,0), rule:"two-runners",
    q:"주자 둘이 <b>2루에 함께</b> 서 있고 수비가 둘 다 태그했다. 원칙적으로 아웃은?",
  ...
  { tag:"누의 공과", outs:1, runners:R(0,0,1), rule:"appeal",
    q:"주자가 <b>3루를 밟지 않고</b> 홈에 들어와 득점했다. 수비가 아웃으로 만들려면?",
